...
export const Sidebar = () => {
  ...
  useEffect(() => {
    const checkActiveSearch = async () => {
      ...
      const {
        data: searches
      } = await supabase.from('lead_finder_searches').select('*').eq('user_id', user.id).eq('status', 'running').order('created_at', {
        ascending: false
      }).limit(1);
    ...
    const channel = supabase.channel('active_searches').on('postgres_changes', {
      event: '*',
      schema: 'public',
  ...
  return <>
      {/* Mobile Menu Button */}
      <Button variant="ghost" size="icon" className="fixed top-3 left-3 z-50 lg:hidden bg-background/80 backdrop-blur-sm border shadow-sm" onClick={() => setIsMobileOpen(!isMobileOpen)}>
      ...
      {/* Sidebar */}
      <div className={cn("flex h-full flex-col border-r bg-card transition-all duration-300", "fixed lg:relative inset-y-0 left-0 z-50", isCollapsed ? "lg:w-16" : "w-64",
    // Mobile: hidden by default, slide in when open
    isMobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
        <div className="flex h-16 items-center border-b px-3 justify-between">
        {!isCollapsed && <h1 className="text-xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">
            LeadGeni
          </h1>}
        <Button variant="ghost" size="icon" onClick={() => setIsCollapsed(!isCollapsed)} className="h-8 w-8 hidden md:flex">
          {isCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
        </Button>
      </div>
      
      {/* Global Search Indicator */}
      {activeSearch && <div className={cn("mx-3 mt-4 p-3 rounded-lg bg-primary/10 border border-primary/20 transition-colors group", isCollapsed && "p-2")}>
          {isCollapsed ? <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex justify-center cursor-pointer" onClick={() => navigate('/lead-finder')}>
            ...
            </Tooltip> : <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="ghost" className="h-6 w-6 p-0 opacity-0 group-hover:opacity-100" onClick={async e => {
              e.stopPropagation();
              ...
              <div className="space-y-1">
                ...
                width: `${activeSearch.progress}%`
              }} />
                </div>
              </div>
              <button onClick={() => navigate('/lead-finder')} className="w-full text-xs text-primary hover:underline text-left font-medium">
                View Details →
              </button>
            </div>}
        </div>}
      ...
      <nav className="flex-1 space-y-1 px-3 py-4 overflow-y-auto">
        {navigation.map(item => {
          ...
          if (item.children) {
            return <Collapsible key={item.name} defaultOpen={isGroupActive}>
                <CollapsibleTrigger asChild>
                  <button className={cn("flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors", isGroupActive ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground", isCollapsed && "justify-center")}>
                ...
                {!isCollapsed && <CollapsibleContent className="space-y-1 mt-1 ml-4">
                  ...
                  let pendingCount = 0;
                  if (pendingCounts) {
                    if (child.href === '/deals') pendingCount = pendingCounts.deals;else if (child.href === '/sequences') pendingCount = pendingCounts.sequences;else if (child.href === '/company-sequences') pendingCount = pendingCounts.campaigns;else if (child.href === '/events') pendingCount = pendingCounts.events;
                  }
                  return <Link key={child.name} to={child.href} className={cn("flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors relative", isChildActive ? "bg-primary text-primary-foreground font-medium" : "text-muted-foreground hover:bg-muted hover:text-foreground")}>
                          <child.icon className="h-4 w-4 shrink-0" />
                          
                          {pendingCount > 0 && <Badge variant="secondary" className="h-5 min-w-5 px-1 text-xs font-semibold bg-destructive text-destructive-foreground animate-pulse">
          ...
          const linkContent = <Link key={item.name} to={item.href} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors relative", isActive ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground", isCollapsed && "justify-center")}>
              <item.icon className="h-5 w-5 shrink-0" />
              {!isCollapsed && <span className="flex-1">{item.name}</span>}
              {pendingCount > 0 && !isCollapsed && <Badge variant="secondary" className="h-5 min-w-5 px-1 text-xs font-semibold bg-destructive text-destructive-foreground animate-pulse">
                  {pendingCount > 99 ? '99+' : pendingCount}
                </Badge>}
              {pendingCount > 0 && isCollapsed && <span className="absolute -top-1 -right-1 h-3 w-3 bg-destructive rounded-full border-2 border-sidebar animate-pulse" />}
            </Link>;
          if (isCollapsed) {
            return <Tooltip key={item.name} delayDuration={0}>
                <TooltipTrigger asChild>
                  {linkContent}
                </TooltipTrigger>
                <TooltipContent side="right">
      ...
      {user && <div className="border-t p-3">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className={cn("w-full h-auto p-3", isCollapsed ? "justify-center" : "justify-start gap-3")}>
                <Avatar className="h-8 w-8 shrink-0">
                  <AvatarFallback className="bg-primary text-primary-foreground">
                    {getUserInitials()}
                  </AvatarFallback>
                </Avatar>
                {!isCollapsed && <div className="flex flex-col items-start text-sm">
