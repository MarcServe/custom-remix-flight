...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (connError || !connection) {
      throw new Error('Gmail connection not found. Please connect your Gmail account.');
    }
    ...
    console.log(`Found ${sentEmails?.length || 0} sent emails to check for replies`);
    ...
    for (const email of sentEmails || []) {
      try {
        ...
        if (!threadResponse.ok) {
          console.log(`Could not fetch thread ${gmailThreadId}:`, await threadResponse.text());
          continue;
        }
        ...
        for (const message of messages) {
          ...
          console.log(`Found reply from ${fromEmail} in thread ${gmailThreadId}`);
          ...
          if (threadError) {
            console.error('Error storing reply:', threadError);
            continue;
          }
        ...
        console.error(`Error processing thread for email ${email.id}:`, threadError);
      }
    }
